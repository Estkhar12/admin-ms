  ...
  try {
    ...
    if (discount.isDeleted) {
      return res.status(403).json({
        error:
          "This discount has been deleted, you cannot update it's property",
      });
    }
    ...
    if (!value || isNaN(value) || value < 0 || value >= 100) {
      return res
        .status(400)
        .json({ error: 'Please provide a valid value between 0 to 100' });
    }
          ...
          if (isValidObjectId(productId)) {
            ...
            if (product) {
              if (discount.type === 'price') {
                ...
                product.platformDiscount = value;
                product.discountedPrice =
                  product.mrp - product.mrp * (value / 100);
                await product.save();
              }
            }
          }
          ...
          if (isValidObjectId(bundleId)) {
            ...
            if (bundle && discount.type === 'price') {
              ...
              bundle.discountedPrice =
                bundle.price - bundle.price * (value / 100);
              await bundle.save();
            }
          }
        })
    ...
    return res
      .status(200)
      .json({ message: 'Updated successfully', data: discount });
  } catch (error) {
